...
{   
    ...
    SubShader
    {
        ...
        float SobelNormals(float2 uv, float2 texelSize)
        {
            ...
            {
                float2 sampleUV = uv + offsets[i] * texelSize;
                float3 d = SampleSceneNormals(sampleUV);
        ...
        float MaskBased(float2 uv, float2 texelSize)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                float2 sampleUV = uv + texelSize.xy * dOffsets[i];
        ...
        Pass
        {
            ...
            float4 Frag (Varyings input) : SV_Target
            {
                ...
                // edge = edge * Mask(input.texcoord).r;
                // return float4(depthEdge, depthEdge, depthEdge, 1);
