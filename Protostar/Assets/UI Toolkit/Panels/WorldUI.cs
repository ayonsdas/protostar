...
public class SimpleWorldUI : MonoBehaviour
{
    ...
    [Tooltip("If true, the UI will only show once and won't reappear again")]
    public bool oneTimeDisplay = false;
    [Tooltip("If true, the UI can be dismissed by the player using the 'Dismiss' input action")]
    ...
    private void OnEnable()
    {
        ...
        else
        {
            ...
            playerInput.actions["Dismiss"].performed += HandleDismiss;
        }
    }
    ...
    {
        if (!canvas) return;
        // If one time display is enabled and the UI has already been shown, don't show it again
        if (oneTimeDisplay && hasBeenShown) return;
    ...
    private void HandleDismiss(InputAction.CallbackContext context)
    {
        ...
        if (dismissable && canvas != null && canvas.gameObject.activeSelf)
        {
            HideUI();
        }
    }
